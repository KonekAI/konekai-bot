...
# print(storyTest("What color is the main character's hood?"))
# print(storyTest("Who ate little red riding hood's grandmother?"))
# print(storyTest("What food was little red riding hood bringing her grandmother?"))
# print(storyTest("How did the wolf die?"))

def parseWikiText(wikiTitle: str):
    wikiObject = wikipedia.page(wikiTitle)
    text = wikiObject.content
    text = re.sub(r'==.*?==+', '', text)
    text = text.replace('\n', '')
    # print(text)
    return text
...
pythonText = parseWikiText("Python (programming language)")


def pyTest(question: str):
    payload = {
        "documents": [pythonText],
        "question": question,
        "search_model": "ada",
        "model": "curie",
        "examples_context": "In 2017, U.S. life expectancy was 78.6 years. Atul Gawande has written about the decline of end of life health care in America.",
